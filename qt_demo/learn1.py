...
class GUi(QMainWindow):
    ...
    def initUI(self):
        ...
        # 设置状态消息栏文本
        self.statusBar().showMessage("底部状态栏文本")
        ...
        sec_action = QAction('第二个文件', self)
        # 添加一个行为到菜单
        file_menu.addAction(new_action)
        file_menu.addAction(sec_action)

        # 更新状态栏文本
        new_action.setStatusTip('打开新的文件')
        sec_action.setStatusTip('第二个文件')
    ...
    def add_position_layout(self):
        label = QLabel("第0个标签", self)
        # self.setCentralWidget(label) # 通过 setCentralWidget() 方法将部件添加到主窗口中
